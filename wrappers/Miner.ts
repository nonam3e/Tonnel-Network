...
export function tonnelConfigToCell(config: MinerConfig): Cell {
    ...
    dict.set(
        BigInt(0),
        beginCell()
            .storeUint(43859932230369129483580312926473830336086498799745261185663267638134570341235n, 256)
            .endCell()
    );
    return beginCell()
        .storeUint(0, 32)
...
export class Miner implements Contract {
        ...
        const input_cell = beginCell()
            ...
            .storeRef(
                beginCell()
                    ...
                    .storeRef(beginCell().storeUint(opts.output_commitment, 256).endCell())
                    .storeRef(opts.proof_withdraw)
                    .endCell()
            )
            .storeRef(
                beginCell()
    ...
    async getRewardNullifier(provider: ContractProvider, nullifier: bigint) {
        const result = await provider.get('get_reward_nullifiers', [{ type: 'int', value: nullifier }]);
        return result.stack.readBoolean();
    }

    async getaccountNullifiers(provider: ContractProvider, nullifier: bigint) {
        const result = await provider.get('get_account_nullifiers', [{ type: 'int', value: nullifier }]);
        return result.stack.readBoolean();
    }
    ...
    async getCheckVerify(provider: ContractProvider, cell: Cell, reward = false) {
        ...
        const result = await provider.get(funcname, [{ type: 'slice', cell: cell } as TupleItemSlice]);
