...
export function drillConfigToCell(config: DrillConfig): Cell {
    return beginCell()
        ...
        .storeRef(
            beginCell()
                .storeAddress(config.rewardMinterAddress)
                .storeAddress(config.lpMinterAddress)
                .storeRef(config.lpBytecode)
                .endCell()
        )
        .storeRef(beginCell().storeUint(0, 64).storeUint(0, 256).storeUint(0, 256).endCell())
        .endCell();
}
