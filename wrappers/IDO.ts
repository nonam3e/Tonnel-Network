...
export function IDOConfigToCell(config: IDOConfig): Cell {
    ...
    config.referrals.forEach((item) => {
        ...
        empty.set(
            ...
            beginCell()
                .storeUint(getCashBack(item.batch), 8)
                .storeUint(getReferral(item.batch), 8)
    ...
    return beginCell()
        ...
        .storeAddress(config.TONNEL_MASTER)
        .endCell();
}
...
export class IDO implements Contract {
    ...
    async sendBuyTONNEL(provider: ContractProvider, via: Sender, value: bigint, referralID: string) {
        await provider.internal(via, {
            ...
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginCell()
                .storeUint(Opcodes.buy_TONNEL, 32)
                .storeUint(0, 64)
                .storeUint(BigInt('0x' + beginCell().storeStringTail(referralID).endCell().hash().toString('hex')), 256)
                .endCell(),
        });
    }
    ...
    async sendSetRef(
        ...
        await provider.internal(via, {
            ...
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: beginCell()
                .storeUint(Opcodes.set_ref, 32)
                .storeUint(0, 64)
                .storeUint(
                    BigInt('0x' + beginCell().storeStringTail(opts.referralID).endCell().hash().toString('hex')),
                    256
                )
    ...
    async getReferral(provider: ContractProvider, referrerId: string) {
        try {
            const result = await provider.get('get_referral', [
                {
                    type: 'int',
                    value: BigInt('0x' + beginCell().storeStringTail(referrerId).endCell().hash().toString('hex')),
                },
            ]);
