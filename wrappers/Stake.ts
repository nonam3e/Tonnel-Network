...
export function tonnelConfigToCell(config: StakeConfig): Cell {
    return beginCell()
        .storeDict(null)
        .storeRef(beginCell().storeAddress(config.jettonMinterAddress).storeRef(config.jettonWalletBytecode).endCell())
        .storeRef(beginCell().storeAddress(config.admin).endCell())
        .endCell();
}
