    ...
    return beginCell()
        ...
        .storeRef(
            beginCell()
                ...
                .storeDict(null)
                .storeRef(
                    beginCell().storeUint(0, 8).storeCoins(toNano('1')).storeDict(discounts).storeDict(null).endCell()
                )
                .endCell()
        )
        .endCell();
...
export class ZKNFTCollection implements Contract {
        ...
        const inputCell = beginCell()
            ...
            .storeRef(
                beginCell()
                    ...
                    .storeRef(beginCell().storeUint(opts.newRoot, 256).endCell())
                    .storeRef(beginCell().storeRef(opts.a).storeRef(opts.b).storeRef(opts.c).endCell())
                    .endCell()
            )
            .endCell();
    ...
    async getRootKnown(provider: ContractProvider, root: bigint) {
        const result = await provider.get('get_root_known', [{ type: 'int', value: root }]);
        return result.stack.readNumber();
    }
