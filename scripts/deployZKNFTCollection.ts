...
export async function run(provider: NetworkProvider) {
    ...
    const jettonTonnel = provider.open(
        JettonMinter.createFromAddress(Address.parse('EQAMcImLBgZHazWmradz51pI0uHZwvxMONlMQy0QwQTQInD5'))
    );
    ...
    // );
    const nftCollection = provider.open(
        ZKNFTCollection.createFromAddress(Address.parse('EQBVa8wuT_P8faSnXDE-HW9DDZu0u_05pfERigXwH7NvsASs'))
    );
    ...
    const jettonWalletMinter = provider.open(
        JettonWallet.createFromAddress(
            await jettonTonnel.getWalletAddress(Address.parse('EQBmVo--5CGcB1YdclgIUvUY-949a0ivzC1Cw9_J3l7ayxnT'))
        )
    );
