...
import { Address, beginCell, Builder, Cell, Sender, Slice, toNano } from '@ton/core';
...
const reward = async function (
    ...
    depositDataEvents: any[],
    withdrawalDataEvents: any[],
    depositTree: any,
    withdrawalTree: any
) {
    const newAmount =
        BigInt(account.amount) + BigInt(rate) * BigInt(note.withdrawBlock - note.depositBlock) - BigInt(fee);
    const newAccount = new Account({ amount: newAmount });
    console.log(note.commitment);

    const depositItem = depositDataEvents.findIndex(
        (x) => BigInt(x) == get_tonnel_tree_leaf(note.commitment, note.instance, note.depositBlock)
    );
    if (depositItem === -1) {
    ...
    const withdrawalItem = withdrawalDataEvents.findIndex(
        (x) => BigInt(x) == get_tonnel_tree_leaf(note.nullifierHash, note.instance, note.withdrawBlock)
    );
    if (withdrawalItem === -1) {
...
const withdraw = async function (
    ...
    recipient: Address,
    fee = 0,
    accountCommitments: any[] | undefined
) {
...
const sliceHash = (args: [Address, bigint, number]): bigint => {
...
async function batchTreeUpdate(tree: any, events: any[], isDeposit: boolean) {
    ...
    const leaves = events.map((e) =>
        mimcHash3(
            get32BitsOfInstance(e.instance),
    ...
    const input = {
        ...
        hashes: events.map((e) => BigNumber.from(isDeposit ? e.note.commitment : e.note.nullifierHash).toString()),
...
describe('TonnelTree', () => {
    ...
    beforeEach(async () => {
        ...
        tonnelJettonMaster = blockchain.openContract(
    ...
    async function doDeposit(tree: MerkleTree, note: any, sender: any) {
        ...
        const depositResult = await tonnel.sendDeposit(sender.getSender(), {
            ...
            //     )
            //     .endCell()
            payload: Cell.fromBoc(
                Buffer.from(
                    '',
                    'hex'
                )
            )[0],
        });
    ...
    it('should init Merkle and then deposit', async () => {
        ...
        let encodedSliceArray: Slice[] = [];
        for (let i = 0; i < 32; i += 3) {
            if (i < 30) {
                encodedSliceArray.push(
                    beginCell()
                        .storeUint(BigInt(input.hashes[i]), 256)
                        .storeUint(BigInt(input.instances[i]), 32)
                        .storeUint(BigInt(input.blocks[i]), 32)
                        .storeUint(BigInt(input.hashes[i + 1]), 256)
                        .storeUint(BigInt(input.instances[i + 1]), 32)
                        .storeUint(BigInt(input.blocks[i + 1]), 32)
                        .storeUint(BigInt(input.hashes[i + 2]), 256)
                        .storeUint(BigInt(input.instances[i + 2]), 32)
                        .storeUint(BigInt(input.blocks[i + 2]), 32)
                        .endCell()
                        .beginParse()
                );
            } else {
        ...
        const { args, account } = await reward(
            ...
            100,
            [],
            treeTONNEL_TREE_deposit.elements,
            treeTONNEL_TREE_withdraw.elements,
            treeTONNEL_TREE_deposit,
            treeTONNEL_TREE_withdraw
        );
        ...
        const res = await tonnelTree.sendRewardRequest(owner.getSender(), {
            value: toNano('0.24'),
            reward_payload: beginCell()
                .storeUint(BigInt(args.depositRoot), 256)
                .storeUint(BigInt(args.withdrawalRoot), 256)
                .storeRef(
                    beginCell()
                        .storeUint(100, 32)
                        .storeCoins(0)
                        ...
                        .storeRef(
                            beginCell()
                                .storeUint(BigInt(args.account.inputNullifierHash), 256)
                                .storeUint(BigInt(args.account.outputRoot), 256)
                                .storeUint(BigInt(args.account.outputPathIndices), 32)
                                .storeUint(BigInt(args.account.outputCommitment), 256)
                                .endCell()
        ...
        const accountTree = new MerkleTree(20, [args.account.outputCommitment], {
            hashFunction: mimcHash2,
            zeroElement: '21663839004416932945382355908790599225266501822907911457504978515578255421292',
        });
        ...
        const res2 = await tonnelTree.sendRewardRequest(owner.getSender(), {
            value: toNano('0.36'),
            reward_payload: beginCell()
                .storeUint(BigInt(args3.depositRoot), 256)
                .storeUint(BigInt(args3.withdrawalRoot), 256)
                .storeRef(
                    beginCell()
                        .storeUint(100, 32)
                        .storeCoins(0)
                        ...
                        .storeRef(
                            beginCell()
                                .storeUint(BigInt(args3.account.inputNullifierHash), 256)
        ...
        const { args: args2, account: account2 } = await withdraw(
            account,
