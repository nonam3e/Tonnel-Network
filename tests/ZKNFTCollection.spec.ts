...
import { groth16, parseG1Func, parseG2Func, rbuffer, toBigIntLE, unstringifyBigInts } from '../utils/circuit';
...
describe('ZKNFTCollection', () => {
    ...
    beforeEach(async () => {
        ...
        let deployJettonResult = await jettonMinter.sendDeploy(
            (await blockchain.treasury('owner')).getSender(),
            toNano('0.05')
        );
    ...
    const mintNFT = async (id: number, secret: bigint, tree: MerkleTree) => {
        ...
        console.log(input);
        const time = Date.now();
        let { proof, publicSignals } = await groth16.fullProve(input, wasmPathInsert, zkeyPathInsert);
        ...
        let mintResult = await nftCollection.sendMintOwner(owner.getSender(), {
            ...
            payload: beginCell()
                .storeUint(BigInt(commitment), 256)
                .storeUint(id, 32)
        ...
        const nftItemContract = blockchain.openContract(NFTItem.createFromAddress(nftAddress));
    ...
    it('should deploy and then mint', async () => {
        ...
        const burnJettonWallet = blockchain.openContract(
            JettonWallet.createFromAddress(
                await jettonMinter.getWalletAddress(Address.parse('EQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAM9c'))
            )
        );
        for (let i = 1; i < 50; i++) {
            ...
            const resultMint = await jettonMinter.sendMint(owner.getSender(), {
            ...
            let purchaseResult = await jettonWalletSender.sendTransfer(sender.getSender(), {
                ...
                jettonAmount: toNano(prices[i < 33 ? i - 1 : 32]),
    ...
    it('should deploy and then mint and reveal', async () => {
        ...
        let { proof: proofInsert, publicSignals: publicSignalsInsert } = await groth16.fullProve(
            input2,
            wasmPathInsert,
            zkeyPathInsert
        );
