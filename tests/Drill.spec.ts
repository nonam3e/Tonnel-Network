...
describe('Drill', () => {
    ...
    beforeEach(async () => {
        ...
        rewardMinter = blockchain.openContract(
        ...
        lpMinter = blockchain.openContract(
            JettonMinter.createFromConfig(
                {
                    adminAddress: owner.address,
                    content: 'a',
    ...
    it('should work', async () => {
        ...
        const lpWalletSenders: SandboxContract<JettonWallet>[] = [];
        const rewardWalletSenders: SandboxContract<JettonWallet>[] = [];
        for (let i = 0; i < 10; i++) {
            lpWalletSenders.push(
                blockchain.openContract(
                    JettonWallet.createFromAddress(await lpMinter.getWalletAddress(senders[i].address))
                )
            );
            rewardWalletSenders.push(
                blockchain.openContract(
